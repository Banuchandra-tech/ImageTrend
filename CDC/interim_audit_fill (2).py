...
# MAGIC %md
# MAGIC # Interim audit-column fill — the bridge until the source capture instances are rebuilt
# MAGIC
# MAGIC **Runs as the task AFTER `stage_compact` in the CDC job.** It patches the one thing CDC cannot
# MAGIC deliver today: the audit/business-key columns (`ModifiedOn`, `CreatedOn`, `CreatedBy`,
# MAGIC `GlobalIdentifier`, `FormID`, …) that the frozen SQL Server capture instances omit, so every CDC
# MAGIC insert/update lands them as NULL in Bronze.
# MAGIC
...
# MAGIC Writes Bronze; read-only against the source.
...
dbutils.widgets.text("missing_cols_table", "bronze._cdc_missing_columns")
dbutils.widgets.text("detail_table", "control.bronze_pipeline_execution_detail")
dbutils.widgets.text("report_table", "bronze._cdc_interim_fill_report")
