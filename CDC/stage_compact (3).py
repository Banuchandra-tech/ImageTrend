...
# MAGIC **boundary-claimed** slice of `medallion.bronze._cdc_stage`, groups by `(tenant, schema, table)`, and for each table
# MAGIC does a **single-pass latest-op-wins** MERGE into `medallion.bronze.<table>`. It has **no Event Hub offset to advance** —
# MAGIC a slow run just means Bronze is one cycle behind; the 2026-06-30 checkpoint stall is structurally impossible here.
...
dbutils.widgets.text("stage_table", "medallion.bronze._cdc_stage")
dbutils.widgets.text("cursor_table", "medallion.control.cdc_stage_compaction_cursor")
...
dbutils.widgets.text("deadletter_table", "medallion.bronze._cdc_stream_deadletter")
# Control/audit tables (parameterized so a non-blue catalog run cannot write into prod control tables)
dbutils.widgets.text("batch_watermark_table", "medallion.control.batch_watermark")
dbutils.widgets.text("table_watermark_table", "medallion.control.bronze_table_watermark")
dbutils.widgets.text("execution_table", "medallion.control.bronze_pipeline_execution")
dbutils.widgets.text("execution_detail_table", "medallion.control.bronze_pipeline_execution_detail")
dbutils.widgets.text("promotion_gate_table", "medallion.control.bronze_batch_watermark")
# --- sliding-window (late-arrival) controls — OFF by default; green sets them, blue keeps current behavior ---
...
# on. Running with per-table batch_ids orphaned any non-max table from bronze_batch_watermark. Forced
...
def write_bronze_watermarks(*, success_updates):
    """Advance control.batch_watermark (Silver keystone) + control.bronze_table_watermark, one chunked MERGE each (#7)."""
...
def write_bronze_run_audit(*, tenant_id, run_batch, run_start_ts, run_end_ts, pipeline_version,
    ...
    """Run-level audit: control.bronze_pipeline_execution(_detail) + the promotion gate control.bronze_batch_watermark."""
    ...
    try:
        if n_fail == 0 and success_updates:
            ...
            # newest (out-of-order completion, or a smaller per-client/per-method id) left its Bronze
...
# more exist in 2-3 schemas) and would resolve to the wrong bronze table.
REGISTRY = {}
for r in reg_rows:
...
def business_schema(bronze_fqn):
    """Parse schema for the Debezium payload, derived from the bronze table.

    Temporal columns are read as STRING, never as their bronze type: Debezium serializes SQL Server
...
def payload_columns(raw):
    ...
    *captured* null is still serialized as `"col":null` — absence therefore means "not captured").
    ...
    from_json types the parse schema off the bronze table, so an absent key parses as NULL. Feeding
    ...
    left untouched on UPDATE (bronze keeps the full-load value) rather than being nulled out.
    """
...
def compact_one_table(row):
    ...
    # wrong bronze table (incident/patient/fieldvalue and 7 more exist in 2-3 schemas).
    ...
    try:
        ...
        # lowercase makes the stored key canonical -> no case-dupes and Bronze matches the full-load/blue
        ...
        # is SQL_Latin1_General_CP1_CI_AS (case-INsensitive), and the two paths into Bronze disagree on
        ...
        (DeltaTable.forName(spark, bronze_fqn).alias("t").merge(write_df.alias("s"), cond)
...
# ---- CDC lag observability: TRUE source->bronze lag from Debezium __source_ts_ms, captured BEFORE the stage
...
try:
    ...
    (lag.withColumn("run_id", F.lit(RUN_ID)).withColumn("run_ts", F.current_timestamp())
        ...
        .write.mode("append").option("mergeSchema", "true").saveAsTable(f"{TCAT}.control.cdc_lag_audit"))
except Exception as e:
...
write_bronze_watermarks(success_updates=success_updates)
...
exec_id = write_bronze_run_audit(tenant_id=TENANT, run_batch=run_batch_for_audit, run_start_ts=run_start_ts,
              run_end_ts=run_end_ts, pipeline_version="bronze-cdc-compact-1.0",
