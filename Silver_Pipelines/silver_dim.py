...
dbutils.widgets.text("dependency_level", "1")
...
# =====================================================
# PROPAGATE FAILURE TO THE JOB
# =====================================================
# Without this the notebook printed the failure and returned normally, so Databricks
# reported the task as SUCCESS no matter what happened. That is how green ran for a day
# with 215 tables SKIPPED, 0 successful and 0 rows inserted while the DAG showed green,
# and how blue reported SUCCESS while failing 3 tables on every run.
#
# A run that fails must fail. Raising here makes the task status match the audit table,
# which is the only reason anyone would trust either of them.
# FAIL-SOFT (2026-07-23, matches blue): per-table failures are recorded in
# control.silver_pipeline_execution_detail (surface: control.pipeline_failure_ledger)
# and DO NOT abort the run — one broken transform must not block the other ~200.
# The run still fails hard when nothing succeeded at all while failures exist,
# which indicates a systemic problem (catalog/storage down), not a bad SQL file.
if failed_count > 0:
    failed_detail = "; ".join(
        f"{table}: {str(error)[:160]}" for table, error in failed_tables[:10]
    )
    print("=" * 80)
    print(f"⚠️  {failed_count} TABLE(S) FAILED, {blocked_count} blocked (run continues, "
          f"failures ledgered): {failed_detail}")
    print("=" * 80)
    if success_count == 0 and skipped_count == 0:
        raise Exception(
            f"Silver pipeline SYSTEMIC FAILURE (dependency_level={DEPENDENCY_LEVEL}): "
            f"every attempted table failed ({failed_count}), nothing succeeded or skipped."
        )

# A run in which every table SKIPPED with nothing inserted is AMBIGUOUS: it is either an
# upstream break (bronze did not advance control.bronze_batch_watermark, so silver sees no
# new batch - the failure that had green skipping 215 tables while reporting green) OR a
# perfectly healthy no-op re-run after the current batch was already drained. Silver cannot
# tell those apart on its own, so this WARNS loudly rather than raising - raising here would
# fail every legitimate "already caught up" run and break the ADF chain on a no-op. The hard
# failure signal lives upstream: the bronze ingestion job fails if a client does not ingest.
if success_count == 0 and skipped_count == len(table_state) and len(table_state) > 0:
    print("=" * 80)
    print(f"⚠️  WARNING: silver did NO WORK (dependency_level={DEPENDENCY_LEVEL}): all "
          f"{skipped_count} tables SKIPPED, 0 inserted.")
    print("    Healthy if the current bronze batch was already processed. If bronze JUST ran "
          "in this cycle, this means it did not advance control.bronze_batch_watermark - "
          "check the bronze ingestion job for every active client.")
    print("=" * 80)